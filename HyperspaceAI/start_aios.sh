...
# Function to display logo (replaced with new curl command)
display_logo() {
...
get_private_key() {
    ...
    log_message "${GREEN}Private key saved as my.pem with correct permissions.${RESET}"
}
...
check_and_install_docker() {
    if ! command -v docker &> /dev/null; then
        ...
        retry apt-get install -y apt-transport-https ca-certificates curl software-properties-common || handle_error "Failed to install dependency packages."
...
# Function to wait for container initialization
wait_for_container_to_start() {
    log_message "${CYAN}Waiting for container initialization...${RESET}"
    sleep 60
}

# Function to check daemon status inside the container
check_daemon_status() {
    log_message "${BLUE}Checking daemon status in the container...${RESET}"
...
check_hive_points() {
    ...
    docker exec -i aios-container /app/aios-cli hive points || log_message "${RED}Unable to get Hive points.${RESET}"
    log_message "${GREEN}Hive points check completed.${RESET}"
}

# Function to get current signed-in keys
get_current_signed_in_keys() {
    log_message "${BLUE}Getting current signed-in keys...${RESET}"
    docker exec -i aios-container /app/aios-cli hive whoami || handle_error "Failed to get current signed-in keys."
}

# Function to cleanup package lists
cleanup_package_lists() {
    log_message "${BLUE}Cleaning up package lists...${RESET}"
    sudo rm -rf /var/lib/apt/lists/* || handle_error "Failed to cleanup package lists."
}
...
# Loop to repeat execution hourly
while true; do
    log_message "${CYAN}Restarting process every 1 hour...${RESET}"

    docker exec -i aios-container /app/aios-cli kill || log_message "${RED}Failed to kill daemon process.${RESET}"
    ...
    log_message "${GREEN}Loop completed. Waiting for 1 hour...${RESET}"
    sleep 3600
done &